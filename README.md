...
(на данный момент админ-панель находится в разработке, и имеет возможность только смотреть конфиг и сессии прокси-сервера)
...
# =OFF - сборка компонента выключена
# =ON - сборка компонента включена
...
Если вы включайте поддержку админ панели, то сгенерируйте сертификаты с помощью скрипта create_certificates.sh.

Использование create_certificates.sh:

```bash
./create_certificates.sh <directory>
```

После выполнения в указанной вами дериктории появятся server.crt и server.key

(Если вы запускайте прокси из дериктории /build, то прокси сам вызовет скрипт для генерации сертификатов, если не найдет их)
